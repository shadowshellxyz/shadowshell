...
class LlmIntentionRecognizer(Starter, IntentionRecognizer):
    """
    LLM-based intention recognizer — classifies user input into intent codes by walking the XoT tree.
    @author: shadowshell<shadowshell@foxmail.com>
    """

    @function_monitor(class_name)
    def __init__(self, app_dir, llm_config, sop_path):
        super().__init__(app_dir)
        self.llm = LlmClient(llm_config)
        self.init_messages()
        self.xot = XoT(app_dir)
        self.xot.build(sop_path, os.path.basename(sop_path))
...
# LlmIntentionRecognizer().recognize()
